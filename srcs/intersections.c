...
int inter_cy(t_vector camera, double t_tmp, t_vector direction, t_cylinder *cylinder, t_vector P, t_vector N, double t)
{
	t_vector point = add_min_operation('+', camera, mult_operation('*', t_tmp, direction));
...
int intersection_cy(t_all *all, t_vector camera, t_vector direction, t_cylinder *cylinder, t_vector *P, t_vector *N, double *t)
{
	...
	if (delta < 0)
		return (-1);
	double t1 = (-b - sqrt(delta)) / (2.0 * a);
	double t2 = (-b + sqrt(delta)) / (2.0 * a); // intentsity must be higher -> direct light!
	...
	if (t1 >= 0)
		...
		{ // lightness here
			...
			// cylinder->rgb.z = 255;
			// cylinder->bright = 1;
			// 	all->closest.intensity += all->cylinder->br;
				// all->cylinder->br -= 0.1;
			return (1);
		}
	if (t2 >= 0)
